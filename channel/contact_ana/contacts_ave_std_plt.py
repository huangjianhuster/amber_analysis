...
# Load data
datafiles = ['contacts_prod1_frq.tsv', 'contacts_prod2_frq.tsv', 'contacts_prod3_frq.tsv']

# Read to dataframes
def sort_dfs(datafiles):
    dfs = []
    for i in datafiles:
        df = pd.read_csv(i, comment="#", header=None, names=['Res1', 'Res2', 'Freq'], sep='\s+')
        df['new_res2'] = np.array([j[2:] for j in df['Res2']])
        dfs.append(df)
    
    # Find overlapping records in the first column
    overlapping_records = set(dfs[0]['new_res2']).intersection(*[set(df['new_res2']) for df in dfs[1:]])

    # Filter DataFrames to include only overlapping records
    dfs_filtered = [df[df['new_res2'].isin(overlapping_records)] for df in dfs]
    
    final_data = {'new_res2': [], 'Mean': [], 'Std': []}
    # Iterate over overlapping records and calculate mean and std
    for record in overlapping_records:
        record_values = []
        for df in dfs_filtered:
            record_values.extend(df.loc[df['new_res2'] == record, 'Freq'].values)
        ...
        if len(record_values) == 12:
            ...
            final_data['new_res2'].append(record)
...
for df,cutoff_frq,ax in zip([data_df, ], cutoff_frq_list, [axs, ]):
    df_screened = df[df['Mean']>cutoff_frq]
    bar_width = 0.04 * df_screened.shape[0]
    df_screened_copy = df_screened.copy()
    print(df_screened_copy)
    colors = get_colors(df_screened_copy)
    
    res2 = [amino_acid_mapping[i[:3]]+i[4:] for i in df_screened_copy['new_res2']]
    df_screened_copy['res2'] = res2
    
    df_screened_copy.plot(kind='bar', x='res2', y='Mean', yerr='Std', ax=ax, color=colors, legend=False, rot=0, width=bar_width, capsize=5)
...
plt.tight_layout()
plt.savefig("contacts_ave_std.svg")
plt.show()
