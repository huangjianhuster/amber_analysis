...
selectivity_filter = u.select_atoms("protein and resid 469:472 and backbone")
...
# calculation params
box_zsize = 126
x0, y0, z0 = gate_residue_514to518.center_of_mass()
bin_max = box_zsize - z0
bin_min = -1*z0
bin_num = int(box_zsize/2)
...
for ts in u.trajectory:
    ...
    selection_K = "( resname Na+ )  and prop x >= %s and "  \
    ...
    selection_O = "byres  (resname TIP3 WAT and name O OH2 and prop x >= %s and "  \
                "prop x <= %s and prop y >= %s and prop y <= %s )" % (str(round(x0-cutoff)), str(round(x0+cutoff)),
                                                                        str(round(y0-cutoff)), str(round(y0+cutoff)))
    # print(selection)
...
def get_pmf(z_data, bin_num, bin_min, bin_max, saved_name=None):
    ...
    if saved_name:
        np.savetxt(saved_name, out, fmt="%.5f", header="Z-position(Ang)\tPMF(kcal/mol)")
    return out
...
# bin_min, bin_max
ion_pmf = get_pmf(K_z, bin_num=bin_num, bin_min=bin_min, bin_max=bin_max, saved_name=f"{output_suffix}_ion_dG.dat")
ion_density = get_density(K_z, bin_num=bin_num, bin_min=bin_min, bin_max=bin_max, saved_name=f"{output_suffix}_ion_den_b2A_cf4.dat")
water_density = get_density(O_z, bin_num=bin_num, bin_min=bin_min, bin_max=bin_max, saved_name=f"{output_suffix}_wat_den_b2A_cf4.dat")
